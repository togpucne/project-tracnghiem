...
if ($id_monhoc <= 0 || trim($data["ten_baithi"] ?? "") === "") {
    Api::json(["error" => "Thi?u thông tin bài thi"], 400);
}
...
$payload = [
    ...
    "trangthai" => $data["trangthai"] ?? "Ðang m?",
...
if (!$ok) {
    $message = $_SESSION["error"] ?? "Không th? luu bài thi";
...
Api::json([
    "success" => true,
    "message" => $id_baithi > 0 ? "C?p nh?t bài thi thành công" : "Thêm bài thi thành công",
]);
